...
////	Jesper, 2021-05-19
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////	*	Stats for max distance, x/y location, and angle to most distal axon and dendrite compartment
////		was not calculated relative to soma, but relative to origin (which defaults to x = on soma but
////		y on L4/L5 boundary), which caused gobbledygook stats for morphologies with somata in different
////		layers, such as Amanda's VIP INs. Fixed this by relying on Sholl x/y coordinates, which are
////		always centered on the soma.
////	Jesper, 2021-07-30
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////	*	Just like Tweaks -> Matrix limits creates limits for the heatmaps, Sholl limits now provides 
////		user-changeable boundaries for the Sholl analysis.
////	Jesper, 2021-08-01
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////	*	Created Hulls as Poly plots, to avoid the nasty pixellation you get with the Map1 plots when you
////		zoom in too much. Hulls as Poly plots contain polygon drawing objects, so should be manipulated
////		using the Tools menu in the graph (ctrl-T or command-T).
////	Jesper, 2021-08-02
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////	*	Now calculates total area of axon and dendrite hulls.
////	Jesper, 2022-09-04
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////	*	Extracts individual data points from layer analysis bar graphs, to enable stats
////	Jesper, 2023-03-20
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////	*	Added short function qM_tidyAllGraphs for tidying up all graphs
////	*	Added the metrics HullYMin and HullYMax for axons and dendrites
////	Jesper, 2023-10-10
////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Function qM_toggleFlipColours()

	if (Exists("qM_flipColours")==0)
		Variable/G qM_flipColours = 1
	else
		NVAR qM_flipColours
		if (qM_flipColours)
			qM_flipColours = 0
	...
	if (qM_flipColours)
		print "Axon/Dendrite colours will be flipped."
	else
		print "Axon/Dendrite colours will NOT be flipped."
	endif
...
//	Variable	nRows = Ceil(nAll/nCols)
	...
	ModifyGraph hideTrace(yLimits)=2
	ModifyGraph mode=2
...
//	Print "=== Reblending maps ==="
	...
	do
		...
		MapGraphList += "cHullPoly_"+currType+";"
...
Function qM_tidyAllGraphs()

	SVAR		MapGraphList

	Variable nColMax = 7
	if ((itemsinlist(MapGraphList)/6) > nColMax)
		nColMax = (itemsinlist(MapGraphList)/6)
	endif
	JT_ArrangeGraphs2(MapGraphList,6,nColMax)
...
Function doBlend(suff,colorMode,colorScaling)
	...
	if (StringMatch(suff[0],"2"))
		...
		if (qM_noAxon==0)
			if (qM_flipColours)
				ModifyGraph rgb($("AxonHoH_Y_"+suff[2,Inf]))=(65535,0,65535)
			else
				ModifyGraph rgb($("AxonHoH_Y_"+suff[2,Inf]))=(65535,65535,0)
			endif
		endif
		if (qM_flipColours)
			ModifyGraph rgb($("DendrHoH_Y_"+suff[2,Inf]))=(65535,65535,0)
		else
...
Function LoadReconFolder(WhichFolder)
	...
	// Put data in table
	Variable	n = 4*6+6+2+4+1+3+2+2+2+2+4+2+2+4		// WARNING TO SELF! Why am I hard-wiring the number of entries?!
	...
	// Create Convex Hull Poly Collection Plot
	DoWindow/K $("cHullPoly_"+typeName)
	Display yLimits vs xLimits as "Hulls as Poly "+typeName
	DoWindow/C $("cHullPoly_"+typeName)
	ModifyGraph hideTrace(yLimits)=2
//	ModifyGraph nticks=3
	...
	ModifyGraph manTick={0,200,0,0},manMinor={0,50}
	ModifyGraph width={Plan,1,bottom,left}
	TextBox/C/N=text0/J/F=0/B=1/X=0.00/Y=0.00 typeName
	JT_AddResizeButton()
	...
	do
		...
		// Collect AXON Hull of Hull data
		...
		Concatenate "tempWave;",cAxonHoH_Y
		// Collect DENDRITE Hull of Hull data
		...
		tempWave -= 1e-3	// to avoid bug in FindConvexHull
		Concatenate "tempWave;",cDendrHoH_Y
		// Add convex hulls to Convex Hull Poly Collection Plot
		// Axon
		if (!qM_noAxon)
			WAVE HullX = $("Axon_cHullX_"+typeName+"_"+num2str(i+1))
			WAVE HullY = $("Axon_cHullY_"+typeName+"_"+num2str(i+1))
			SetDrawLayer/W=$("cHullPoly_"+typeName) UserBack
			SetDrawEnv/W=$("cHullPoly_"+typeName) xcoord= bottom,ycoord= left,fillfgc= (52428,52425,1,16384),linethick= 0.00
			if (qM_flipColours)
				SetDrawEnv/W=$("cHullPoly_"+typeName) fillfgc= (64536,0,65478,16384)
			endif
			DrawPoly/W=$("cHullPoly_"+typeName) HullX[0],HullY[0],1,1,HullX,HullY
		endif
		// Dendrite
		WAVE HullX = $("Dendr_cHullX_"+typeName+"_"+num2str(i+1))
		WAVE HullY = $("Dendr_cHullY_"+typeName+"_"+num2str(i+1))
		SetDrawLayer/W=$("cHullPoly_"+typeName) UserBack
		SetDrawEnv/W=$("cHullPoly_"+typeName) xcoord= bottom,ycoord= left,fillfgc= (64536,0,65478,16384),linethick= 0.00
		if (qM_flipColours)
			SetDrawEnv/W=$("cHullPoly_"+typeName) fillfgc= (65535,50000,3600,16384)
		endif
		DrawPoly/W=$("cHullPoly_"+typeName) HullX[0],HullY[0],1,1,HullX,HullY
		delayUpdate
		i += 1
	...
	// Add layers to Hulls as Poly plot
	DoWindow/F $("cHullPoly_"+typeName)
	NVAR	AlignOnL4L5
	if (AlignOnL4L5)
		AddLayerBoundaries(typeName,1)
	endif
...
Function CompileLayerAnalysis(typeName,nFiles)
	...
	k = 0 // counts LayerWavesStr data type
	do
		...
		do
			...
			do
				...
				i += 1
			while(i<nFiles)
			Duplicate/O workWave,$("all"+currLayerWave+"_"+typeName+"_"+num2str(j+1))			// Remember for future stats
...
//	Print "nSoma points",nSomaPoints
	...
	NVAR		AlignOnL4L5
	Duplicate/O $("xCoord2"+Suff),$("xSholl"+Suff)	// when aligning on L4/L5 boundary, keep *** SOMA-ALIGNED COORDINATES *** for Sholl analysis
	Duplicate/O $("yCoord2"+Suff),$("ySholl"+Suff)
	WAVE		xCoord2Sh = $("xSholl"+Suff)				// SOMA-ALIGNED COORDINATES
	...
	do
		if (compType[i] == 2)			// 3 = dendrite & Axon, 2 = axon
			...
			AxonShollX[nAxonPoints] = {xCoord2Sh[i]}				// SOMA-ALIGNED COORDINATES: For Sholl analysis, always align on soma
			...
			if (compType2[i]!=SomaCompType)
				...
				DendrShollX[nDendrPoints] = {xCoord2Sh[i]}		// SOMA-ALIGNED COORDINATES: For Sholl analysis, always align on soma
	...
	// Axon ABSOLUTE center (NOT relative to soma)
	WaveStats/Q AxonX
	...
	// Find Axon compartment with max distance FROM THE SOMA
	Duplicate/O AxonShollX,$("compAxonDist"+Suff)
	WAVE		compAxonDist = $("compAxonDist"+Suff)
//	compAxonDist = sqrt(AxonX^2+AxonY^2)								// Prior to 30 July 2021
	compAxonDist = sqrt(AxonShollX^2+AxonShollY^2)					// Bugfix 30 July 2021 -- I am a fuckwit!
	WaveStats/Q compAxonDist
	Variable/G	maxAxonDistComp = V_maxloc
	Variable/G	maxAxonDist = V_max
//	Variable/G	maxAxonDistCompX = AxonX[maxAxonDistComp]			// Prior to 30 July 2021
//	Variable/G	maxAxonDistCompY = AxonY[maxAxonDistComp]
	Variable/G	maxAxonDistCompX = AxonShollX[maxAxonDistComp]	// Bugfix 30 July 2021: This should be in SOMA-RELATIVE coordinates!
	Variable/G	maxAxonDistCompY = AxonShollY[maxAxonDistComp]
	Variable/G	maxAxonDistCompAngle = atan2(maxAxonDistCompY,maxAxonDistCompX)*180/Pi		// This is the angle of straight line to most distal compartment from the soma
//	xCenters[2] = maxAxonDistCompX										// Prior to 30 July 2021
//	yCenters[2] = maxAxonDistCompY
	xCenters[2] = AxonX[maxAxonDistComp]								// What is plotted should still be in ABSOLUTE coordinates, NOT relative to soma!
	yCenters[2] = AxonY[maxAxonDistComp]
	...
	// Find Dendr compartment with max distance FROM THE SOMA
	Duplicate/O DendrX,$("compDendrDist"+Suff)
	WAVE		compDendrDist = $("compDendrDist"+Suff)
//	compDendrDist = sqrt(DendrX^2+DendrY^2)							// Prior to 30 July 2021
	compDendrDist = sqrt(DendrShollX^2+DendrShollY^2)				// Bugfix 30 July 2021
	WaveStats/Q compDendrDist
	Variable/G	maxDendrDistComp = V_maxloc
	Variable/G	maxDendrDist = V_max
//	Variable/G	maxDendrDistCompX = DendrX[maxDendrDistComp]			// Prior to 30 July 2021
//	Variable/G	maxDendrDistCompY = DendrY[maxDendrDistComp]
	Variable/G	maxDendrDistCompX = DendrShollX[maxDendrDistComp]	// Bugfix 30 July 2021: This should be in SOMA-RELATIVE coordinates!
	Variable/G	maxDendrDistCompY = DendrShollY[maxDendrDistComp]
	Variable/G	maxDendrDistCompAngle = atan2(maxDendrDistCompY,maxDendrDistCompX)*180/Pi		// This is the angle of straight line to most distal compartment from the soma
//	xCenters[3] = maxDendrDistCompX										// Prior to 30 July 2021
//	yCenters[3] = maxDendrDistCompY
	xCenters[3] = DendrX[maxDendrDistComp]								// What is plotted should still be in ABSOLUTE coordinates, NOT relative to soma!
	yCenters[3] = DendrY[maxDendrDistComp]
	...
	// Axon center RELATIVE TO SOMA
//	Variable/G	AxonRelXCenter = (AxonXCenter - 0)					// Prior to 30 July 2021
...
//	yCenters[0] = AxonRelYCenter
	WaveStats/Q AxonShollX													// Bugfix 30 July 2021
	Variable/G	AxonRelXCenter = V_avg
	WaveStats/Q AxonShollY
	Variable/G	AxonRelYCenter = V_avg
	Variable/G	AxonCenterDist = sqrt(AxonRelXCenter^2+AxonRelYCenter^2)
	Variable/G	AxonAngle = atan2(AxonRelYCenter,AxonRelXCenter)*180/Pi
	xCenters[0] = AxonXCenter												// What is plotted should still be in ABSOLUTE coordinates, NOT relative to soma!
	yCenters[0] = AxonYCenter

	// Dendrite center RELATIVE TO SOMA
//	Variable/G	DendrRelXCenter = (DendrXCenter - 0)				// Prior to 30 July 2021
...
//	yCenters[1] = DendrRelYCenter
	WaveStats/Q DendrShollX													// Bugfix 30 July 2021
	Variable/G	DendrRelXCenter = V_avg
	WaveStats/Q DendrShollY
	Variable/G	DendrRelYCenter = V_avg
	Variable/G	DendrCenterDist = sqrt(DendrRelXCenter^2+DendrRelYCenter^2)
	Variable/G	DendrAngle = atan2(DendrRelYCenter,DendrRelXCenter)*180/Pi
	xCenters[1] = DendrXCenter												// What is plotted should still be in ABSOLUTE coordinates, NOT relative to soma!
	yCenters[1] = DendrYCenter
	...
	Duplicate/O cHullX,$("Axon_cHullX"+Suff)								// This is the name of the AXON convex hull
	Duplicate/O cHullY,$("Axon_cHullY"+Suff)
	Center = CenterHull(cHullX,cHullY,"Axon",xCoord2LF,yCoord2LF)	// This also creates the matrix map of the AXON convex hull
	...
	Variable/G	AxonHullArea = HullArea
	xCenters[4] = AxonHullCenterX
	yCenters[4] = AxonHullCenterY
	Variable/G	AxonHullYMax = HullYMax										// CenterHull calculates this
	Variable/G	AxonHullYMin = HullYMin										// CenterHull calculates this

	// Dendrite Convex Hull
	FindConvexHull(DendrX,DendrY)
	Duplicate/O cHullX,$("Dendr_cHullX"+Suff)								// This is the name of the DENDRITE convex hull
	Duplicate/O cHullY,$("Dendr_cHullY"+Suff)
	WaveStats/Q	cHullX
	Variable/G	DendrHullWidth = V_max-V_min
	WaveStats/Q	cHullY
	Variable/G	DendrHullHeight = V_max-V_min
	Center = CenterHull(cHullX,cHullY,"Dendr",xCoord2LF,yCoord2LF)	// This also creates the matrix map of the DENDRITE convex hull
	...
	Variable/G	DendrHullArea = HullArea
	xCenters[5] = DendrHullCenterX
	yCenters[5] = DendrHullCenterY
	Variable/G	DendrHullYMax = HullYMax										// CenterHull calculates this
	Variable/G	DendrHullYMin = HullYMin										// CenterHull calculates this
	...
	if (tableIndex==-1)
		...
		Print "Axon hull area",AxonHullArea,"micron^2"
		Print "Dendrite hull area",DendrHullArea,"micron^2"
		Print "Axon hull y-min and y-max:",AxonHullYMin,AxonHullYMax
		Print "Dendrite hull y-min and y-max:",DendrHullYMin,DendrHullYMax
	endif
	...
	VarStr += "AxonHullArea;"
	VarStr += "DendrHullArea;"

	VarStr += "AxonHullYMin;"
	VarStr += "AxonHullYMax;"
	VarStr += "DendrHullYMin;"
	VarStr += "DendrHullYMax;"
...
//	String		currAxonTrace,currDendrTrace
	...
	if (!qM_noAxon)
		...
		Legend/C/N=text0/J/F=0/B=1/X=0.00/Y=0.00 "\\s("+"DendrShollY_"+typeName+") dendr"
	endif
...
Function ShowSholl(Suff,AppendFlag)
	...
	ModifyGraph/W=ShollPlot RGB($("Dendr_Xings"+Suff))=(0,0,65535)
...
Function DoSholl(wX,wY)
	...
	Variable	rMax = 650
	if (Exists("qM_ShollLimits")==1)
		WAVE qM_ShollLimits
		nSteps = qM_ShollLimits[0]
		rMax = qM_ShollLimits[1]
	endif
...
//	Variable	gaussDiam = 5
	Variable	gaussDiam = 25	// Confusing notation: 2*sigma^2 = gaussDiam
...
Function qM_CreateShollLimitsTable()

	print "--- Creating Sholl Limits Table ---"
	print "Set the desired # of circles & radius max limit and then reanalyze."

	if (Exists("qM_ShollLimits")==0)
		Make/O/N=(2) qM_ShollLimits
		qM_ShollLimits = {100,650}
	else
		WAVE	qM_ShollLimits
	endif
	Make/O/T/N=(2) qM_ShollLimitsLabels
	qM_ShollLimitsLabels[0] = "nCircles"
	qM_ShollLimitsLabels[1] = "rMax"
	
	doWindow/K qM_ShollLimitsTable
	Edit/K=1/W=(5,53,300,253) qM_ShollLimitsLabels,qM_ShollLimits as "Sholl limits"
	doWindow/C qM_ShollLimitsTable
	ModifyTable title(qM_ShollLimitsLabels)="Description",title(qM_ShollLimits)="Value"
